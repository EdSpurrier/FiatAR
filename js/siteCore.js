...
function update(dt) {
    if (playerInput) {
        ...
        currentFrame < fpsLock ? currentFrame++ : (scaleAmount = round(1.25 - speedPercent / 4, 2), 
        ...
        playerX < -1 ? (playerX = -1, speed > offRoadMinSpeed && (speed = Util.accelerate(speed, offRoadDecel, dt))) : playerX > 1 && (playerX = 1, 
        ...
        position > playerZ && (currentLapTime && startPosition < playerZ ? lastLapTime = currentLapTime : currentLapTime += dt), 
...
function resetSprites() {
    var n;
    for (n = 0; n < 300; n += 9 + Math.floor(n / 100)) addSprite(n, SPRITES.CYPRESS, 1 + 1.5 * Math.random());
    for (n = 0; n < 300; n += 12 + Math.floor(n / 100)) addSprite(n, SPRITES.CYPRESS, 3 + .5 * Math.random());
    for (n = 0; n < 200; n += 40 + Math.floor(n / 100)) addSprite(n, SPRITES.STONEPINE, -1 - 2 * Math.random());
    for (n = 0; n < 200; n += 10 + Math.floor(n / 100)) addSprite(n, SPRITES.STONEPINE, -3 - 3 * Math.random());
    for (n = 0; n < 200; n += 40 + Math.floor(n / 100)) addSprite(n, SPRITES.ROSEBUSH, -1.2 - 2 * Math.random());
    for (n = 0; n < 200; n += 30 + Math.floor(n / 100)) addSprite(n, SPRITES.SUNFLOWERS, -1.2 - 2 * Math.random());
    ...
    n = 270; n < 600; n += 4 + Math.floor(n / 100)) addSprite(n, SPRITES.CYPRESS, -1 - 2 * Math.random());
    for (n = 270; n < 450; n += 4 + Math.floor(n / 100)) addSprite(n, SPRITES.CYPRESS, 1 + 2 * Math.random());
    ...
    for (addSprite(700, SPRITES.BILLBOARDFIAT, 2), n = 540; n < 720; n += 5 + Math.floor(n / 100)) addSprite(n, SPRITES.STONEPINE, 1.25 + 3 * Math.random());
    for (n = 720; n < 900; n += 5 + Math.floor(n / 100)) addSprite(n, SPRITES.OLIVETREE, 1.25 + 3 * Math.random());
    for (n = 540; n < 900; n += 5 + Math.floor(n / 100)) addSprite(n, SPRITES.SUNFLOWERS, 1.25 + 3 * Math.random());
    ...
    n = 975; n < 1220; n += 10) addSprite(n, SPRITES.CYPRESS, 1 + 1 * Math.random());
    for (addSprite(980, SPRITES.BILLBOARDABARTH, -2), n = 900; n < 1050; n += 40 + Math.floor(n / 100)) addSprite(n, SPRITES.STONEPINE, -1 - 2 * Math.random());
    for (n = 900; n < 1050; n += 10 + Math.floor(n / 100)) addSprite(n, SPRITES.STONEPINE, -3 - 3 * Math.random());
    for (n = 900; n < 1050; n += 40 + Math.floor(n / 100)) addSprite(n, SPRITES.ROSEBUSH, -1.2 - 2 * Math.random());
    for (n = 900; n < 1050; n += 30 + Math.floor(n / 100)) addSprite(n, SPRITES.SUNFLOWERS, -1.2 - 2 * Math.random());
    for (n = 1050; n < 1220; n += 2) addSprite(n, SPRITES.CYPRESS, -1 - 10 * Math.random());
}
    ...
    var $el = {}, timeLine = new TimelineLite(), init = function() {
        ...
        timeLine.fromTo([ $el.game.ready3 ], anim_fast, {
            ...
            onComplete: function() {
                $el.expanded.gameHUDelements.css({
                    display: "block"
                }), raceStarted = !0, playerInputStatus(!0), pauseGame(!1), $el.soundFX.beep.play();
            }
...
!function(root, factory) {
    ...
    return headtrackr.rev = 2, headtrackr.Tracker = function(params) {
        ...
        this.start = function() {
            ...
            !0) : (run = !0, videoElement.addEventListener("playing", starter, !1), !0));
        }, this.stop = function() {
        ...
        for (var avggray, avgr, avgb, avgg, canvasContext = canvas.getContext("2d"), image = canvasContext.getImageData(0, 0, canvas.width, canvas.height), id = image.data, imagesize = image.width * image.height, r = g = b = 0, i = 0; i < imagesize; i++) r += id[4 * i], 
        ...
        function predict(time) {
            var retPos = [];
            if (this.interpolate) for (var step = time / interval, stepLo = step >> 0, ratio = alpha / (1 - alpha), a = (step - stepLo) * ratio, b = 2 + stepLo * ratio, c = 1 + stepLo * ratio, i = 0; i < sl; i++) retPos[i] = a * (sp[i] - sp2[i]) + b * sp[i] - c * sp2[i]; else for (var step = time / interval >> 0, ratio = alpha * step / (1 - alpha), a = 2 + ratio, b = 1 + ratio, i = 0; i < sl; i++) retPos[i] = a * sp[i] - b * sp2[i];
            return retPos;
        }
        ...
        this.initialized = !1, this.interpolate = !1, this.init = function(initPos) {
            ...
            if (positions = [ pos.x, pos.y, pos.z, pos.width, pos.height ], this.initialized) {
                for (var i = 0; i < sl; i++) sp[i] = alpha * positions[i] + (1 - alpha) * sp[i], 
        ...
        for (x = 0, il = imgdata.length; x < il; x += 4) r = imgdata[x + 0] >> 4, g = imgdata[x + 1] >> 4, 
        ...
        var i, j, val, vx, vy, a = [];
        for (i = x; i < w; i++) for (a = data[i], vx = i - x, j = y; j < h; j++) val = a[j], 
        ...
        function getWeights(mh, ch) {
            for (var p, weights = [], i = 0; i < 4096; i++) p = 0 != ch.getBin(i) ? Math.min(mh.getBin(i) / ch.getBin(i), 1) : 0, 
            weights.push(p);
            return weights;
        }
        function getBackProjectionData(imgData, idw, idh, weights, hsMap) {
            ...
            for (x = 0; x < idw; x++) {
                for (a = [], y = 0; y < idh; y++) pos = 4 * (y * idw + x), r = imgData[pos] >> 4, 
        ...
        this.getSearchWindow = function() {
            ...
            for (x = 0; x < w; x++) for (y = 0; y < h; y++) val = Math.floor(255 * weights[x][y]), 
        ...
        this.init = function(inputcanvas) {
            ...
            previousWhitebalances.unshift(result.wb), previousWhitebalances.length == pwbLength)) {
                ...
                max - min < 2 && (_currentDetection = "VJ");
            }
        ...
        this.track = function(facetrackrObj) {
            ...
            if (edgecorrection) {
                var margin = 11, leftDistance = fx - w / 2, rightDistance = this.camwidth_cam - (fx + w / 2), topDistance = fy - h / 2, bottomDistance = this.camheight_cam - (fy + h / 2), onVerticalEdge = leftDistance < margin || rightDistance < margin, onHorizontalEdge = topDistance < margin || bottomDistance < margin;
                if (onHorizontalEdge) if (onVerticalEdge) {
                    var onLeftEdge = leftDistance < margin, onTopEdge = topDistance < margin;
                ...
                } else if (onVerticalEdge) if (leftDistance < margin) {

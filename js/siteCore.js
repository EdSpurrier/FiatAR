...
function checkCameraStatus() {
    cameraAvailable && startMotionTracking();
}
...
function startMotionTracking() {
    htracker = new headtrackr.Tracker({
        ...
        ui: !1,
        headPosition: !1
    }), htracker.init(videoInput, canvasInput), htracker.start(), document.addEventListener("facetrackingEvent", headTrackMotion);
}
...
function update(dt) {
    if (playerInput) {
        ...
        fpsLock > currentFrame ? currentFrame++ : (scaleAmount = round(1.25 - speedPercent / 4, 2), 
        currentFrame = 0, keyLeft ? (rotateAmount = round(3 * motionControllerOutputValue, 2), 
        $canvasElement.css({
            bottom: -round(25 * motionControllerOutputValue, 2)
        }), scaleAmount += round(.2 * motionControllerOutputValue, 2)) : keyRight ? (rotateAmount = -round(3 * motionControllerOutputValue, 2), 
        $canvasElement.css({
            bottom: -round(25 * motionControllerOutputValue, 2)
        }), scaleAmount += round(.2 * motionControllerOutputValue, 2)) : ($canvasElement.css({
            bottom: 0
        }), rotateAmount = 0), $canvasElement.css({
        ...
        speed > car.speed && Util.overlap(playerX, playerW / (alteredOtherCarRatio + alteredOtherCarOffset), car.offset, carW / (alteredOtherCarRatio + alteredOtherCarOffset), .8)) {
            impact1.play(), speed = car.speed * (car.speed / speed), position = Util.increase(car.z, -playerZ, trackLength);
            break;
        }
        ...
        position > playerZ && (currentLapTime && playerZ > startPosition ? lastLapTime = currentLapTime : currentLapTime += dt), 
...
function render() {
    if (gamePaused) Render.player(ctx, width, height, resolution, roadWidth, sprites, speed / maxSpeed, cameraDepth / playerZ / 5, width / 2, height / 2 - cameraDepth / playerZ * Util.interpolate(playerSegment.p1.camera.y, playerSegment.p2.camera.y, playerPercent) * height / 2, speed * (keyLeft ? -1 : keyRight ? 1 : 0), playerSegment.p2.world.y - playerSegment.p1.world.y); else {
        ...
        for (n = drawDistance - 1; n > 0; n--) {
            ...
            segment == playerSegment && Render.player(ctx, width, height, resolution, roadWidth, sprites, speed / maxSpeed, cameraDepth / playerZ / 5, width / 2, height / 2 - cameraDepth / playerZ * Util.interpolate(playerSegment.p1.camera.y, playerSegment.p2.camera.y, playerPercent) * height / 2, speed * (keyLeft ? -1 : keyRight ? 1 : 0), playerSegment.p2.world.y - playerSegment.p1.world.y);
        }
    }
}
...
function resetRoad() {
    ...
    addRoad(10, 50, 10, 0, 17), addRoad(10, 20, 10, 0, 0), addRoad(20, 80, 20, 2, -15), 
    addRoad(10, 40, 10, 0, 0), addRoad(10, 50, 10, -2, 15), addRoad(0, 100, 0, 0, 0), 
    addRoad(20, 80, 20, -2, 15), addRoad(20, 50, 20, 0, 0), addRoad(20, 50, 20, 2, -15), 
...
function resetSprites() {
    var n;
    for (n = 0; 300 > n; n += 9 + Math.floor(n / 100)) addSprite(n, SPRITES.CYPRESS, 1 + 1.5 * Math.random());
    for (n = 0; 300 > n; n += 12 + Math.floor(n / 100)) addSprite(n, SPRITES.CYPRESS, 3 + .5 * Math.random());
    for (n = 0; 200 > n; n += 40 + Math.floor(n / 100)) addSprite(n, SPRITES.STONEPINE, -1 - 2 * Math.random());
    for (n = 0; 200 > n; n += 10 + Math.floor(n / 100)) addSprite(n, SPRITES.STONEPINE, -3 - 3 * Math.random());
    for (n = 0; 200 > n; n += 40 + Math.floor(n / 100)) addSprite(n, SPRITES.ROSEBUSH, -1.2 - 2 * Math.random());
    for (n = 0; 200 > n; n += 30 + Math.floor(n / 100)) addSprite(n, SPRITES.SUNFLOWERS, -1.2 - 2 * Math.random());
    ...
    n = 270; 600 > n; n += 4 + Math.floor(n / 100)) addSprite(n, SPRITES.CYPRESS, -1 - 2 * Math.random());
    for (n = 270; 450 > n; n += 4 + Math.floor(n / 100)) addSprite(n, SPRITES.CYPRESS, 1 + 2 * Math.random());
    ...
    for (addSprite(700, SPRITES.BILLBOARDFIAT, 2), n = 540; 720 > n; n += 5 + Math.floor(n / 100)) addSprite(n, SPRITES.STONEPINE, 1.25 + 3 * Math.random());
    for (n = 720; 900 > n; n += 5 + Math.floor(n / 100)) addSprite(n, SPRITES.OLIVETREE, 1.25 + 3 * Math.random());
    for (n = 540; 900 > n; n += 5 + Math.floor(n / 100)) addSprite(n, SPRITES.SUNFLOWERS, 1.25 + 3 * Math.random());
    for (addSprite(920, SPRITES.FENCE, 1.5), addSprite(920, SPRITES.FENCE, 2.5), addSprite(920, SPRITES.FENCE, 3.5), 
    ...
    n = 975; 1220 > n; n += 10) addSprite(n, SPRITES.CYPRESS, 1 + 1 * Math.random());
    for (addSprite(980, SPRITES.BILLBOARDABARTH, -2), n = 900; 1050 > n; n += 40 + Math.floor(n / 100)) addSprite(n, SPRITES.STONEPINE, -1 - 2 * Math.random());
    for (n = 900; 1050 > n; n += 10 + Math.floor(n / 100)) addSprite(n, SPRITES.STONEPINE, -3 - 3 * Math.random());
    for (n = 900; 1050 > n; n += 40 + Math.floor(n / 100)) addSprite(n, SPRITES.ROSEBUSH, -1.2 - 2 * Math.random());
    for (n = 900; 1050 > n; n += 30 + Math.floor(n / 100)) addSprite(n, SPRITES.SUNFLOWERS, -1.2 - 2 * Math.random());
    for (n = 1050; 1220 > n; n += 2) addSprite(n, SPRITES.CYPRESS, -1 - 10 * Math.random());
}
...
function debugConsole() {
    var $el = {}, consoleLine = 0, log = function($content) {
        ...
        createValue("current-lap-time"), styleConsole(), debugConsole("init"), document.onkeypress = function(e) {
...
function InitMH() {
    ...
    document.getElementById("end-game-cta").addEventListener("click", bgExitHandler, !1);
}

function bgExitHandler(e) {
    Enabler.exit("CTA Exit"), panelExpanded && Enabler.requestCollapse();
}
...
function viewAnimations() {
    var $el = {}, timeLine = new TimelineLite(), init = function() {
        ...
        $el.motionControllerVideo = $("#outputVideo, #motion-track-overlay"), $el.soundFX = {}, 
        ...
        viewStatus = "play", timeLine.clear(), setupGame(), resetPlayerPosition(), carIdle.play(), 
        carRun.play(), carRun.playbackRate = 2, timeLine.to([ $el.expanded.instructions.instruction, $el.expanded.instructions.cars, $el.expanded.instructions.header ], anim_fast_x2, {
            opacity: 0,
            onComplete: function() {
                $el.expanded.instructions.view.css({
                    display: "none"
        ...
        timeLine.clear(), timeLine.fromTo([ $el.expanded.gameHUDniceDriving ], anim_fast, {
            opacity: 0,
            scale: .7
        }, {
            opacity: 1,
...
}, frameNo = 0, firstFrame = !0, keyDown = !1, keyboardControlInstructions = !1, renderCar = !1, Dom = {
    get: function(id) {
...
var Game = {
    ...
    player: function(ctx, width, height, resolution, roadWidth, sprites, speedPercent, scale, destX, destY, steer, updown) {
        ...
        speedPercent > 0 && (bounce = (speedPercent - drivingCarVibrate * Math.random() * speedPercent * resolution) * Util.randomChoice([ -1, 1 ]));
        var sprite;
        ...
        renderCar && Render.sprite(ctx, width, height, resolution, roadWidth, sprites, sprite, scale, destX, destY + bounce, -.5, -1);
    },
...
SPRITES.SCALE = 1.2 * (1 / SPRITES.PLAYER_STRAIGHT.w), SPRITES.BILLBOARDS = [ SPRITES.BILLBOARDABARTH, SPRITES.BILLBOARDFIAT ], 
...
var raceStarted = !1, gamePaused = !1;

!function(root, factory) {
    "object" == typeof exports ? module.exports = factory() : "function" == typeof define && define.amd ? define([], factory) : root.headtrackr = factory();
}(this, function() {
    function supports_video() {
        return !!document.createElement("video").canPlayType;
    }
    function supports_h264_baseline_video() {
        if (!supports_video()) return !1;
    ...
    var headtrackr = {};
    return headtrackr.rev = 2, headtrackr.Tracker = function(params) {
        if (params || (params = {}), void 0 === params.smoothing && (params.smoothing = !0), 
        void 0 === params.retryDetection && (params.retryDetection = !0), void 0 === params.ui && (params.ui = !0), 
        void 0 === params.debug) params.debug = !1; else if ("CANVAS" != params.debug.tagName) params.debug = !1; else var debugContext = params.debug.getContext("2d");
        void 0 === params.detectionInterval && (params.detectionInterval = 20), void 0 === params.fadeVideo && (params.fadeVideo = !1), 
        void 0 === params.cameraOffset && (params.cameraOffset = 11.5), void 0 === params.calcAngles && (params.calcAngles = !1), 
        void 0 === params.headPosition && (params.headPosition = !0);
        var ui, smoother, facetracker, headposition, canvasContext, videoElement, detector, detectionTimer, fov = 0, run = !1, faceFound = !1, firstRun = !0, videoFaded = !1, headDiagonal = [];
        this.status = "", this.stream = void 0;
        var statusEvent = document.createEvent("Event");
        statusEvent.initEvent("headtrackrStatus", !0, !0);
        var headtrackerStatus = function(message) {
            statusEvent.status = message, document.dispatchEvent(statusEvent), this.status = message;
        }.bind(this), insertAltVideo = function(video) {
            if (void 0 === params.altVideo) return !1;
            if (supports_video()) {
                if (params.altVideo.ogv && supports_ogg_theora_video()) video.src = params.altVideo.ogv; else if (params.altVideo.mp4 && supports_h264_baseline_video()) video.src = params.altVideo.mp4; else {
                    if (!params.altVideo.webm || !supports_webm_video()) return !1;
        ...
        this.init = function(video, canvas, setupVideo) {
            if (void 0 === setupVideo || 1 == setupVideo) {
                ...
                window.URL = window.URL || window.webkitURL || window.msURL || window.mozURL, navigator.getUserMedia) {
                    ...
                    window.opera && (window.URL = window.URL || {}, window.URL.createObjectURL || (window.URL.createObjectURL = function(obj) {
                        return obj;
                    })), navigator.getUserMedia(videoSelector, function(stream) {
                        headtrackerStatus("camera found"), this.stream = stream, video.mozCaptureStream ? video.mozSrcObject = stream : video.src = window.URL && window.URL.createObjectURL(stream) || stream, 
                        video.play(), cameraActive = !0, siteCore.apps.viewAnimations.animateInstructionsUpdate("#move-left");
                    }.bind(this), function() {
                        headtrackerStatus("no camera"), siteCore.apps.viewAnimations.animateInstructionsUpdate("#how-to-play-keyboard"), 
                        insertAltVideo(video);
                    });
                } else if (headtrackerStatus("no getUserMedia"), !insertAltVideo(video)) return !1;
                video.addEventListener("playing", function() {
                    video.width > video.height ? video.width = 320 : video.height = 240;
                }, !1);
            }
            videoElement = video, canvasElement = canvas, canvasContext = canvas.getContext("2d"), 
            params.ui && (ui = new headtrackr.Ui()), smoother = new headtrackr.Smoother(.35, params.detectionInterval + 15), 
            this.initialized = !0;
        };
        var track = function() {
            canvasContext.drawImage(videoElement, 0, 0, canvasElement.width, canvasElement.height), 
            void 0 === facetracker && (facetracker = new headtrackr.facetrackr.Tracker({
                debug: params.debug,
                calcAngles: params.calcAngles
            }), facetracker.init(canvasElement)), facetracker.track();
            var faceObj = facetracker.getTrackingObject({
                debug: params.debug
            });
            if ("WB" == faceObj.detection && headtrackerStatus("whitebalance"), firstRun && "VJ" == faceObj.detection && headtrackerStatus("detecting"), 
            0 != faceObj.confidence) {
                if ("VJ" == faceObj.detection) {
                    void 0 === detectionTimer && (detectionTimer = new Date().getTime()), new Date().getTime() - detectionTimer > 5e3 && headtrackerStatus("hints");
                    faceObj.x + faceObj.width / 2, faceObj.y + faceObj.height / 2;
                    params.debug && (debugContext.strokeStyle = "#0000CC", debugContext.strokeRect(faceObj.x, faceObj.y, faceObj.width, faceObj.height));
                }
                if ("CS" == faceObj.detection) {
                    faceObj.x, faceObj.y;
                    if (void 0 !== detectionTimer && (detectionTimer = void 0), params.debug && (debugContext.translate(faceObj.x, faceObj.y), 
                    debugContext.rotate(faceObj.angle - Math.PI / 2), debugContext.strokeStyle = "#00CC00", 
                    debugContext.strokeRect(-(faceObj.width / 2) >> 0, -(faceObj.height / 2) >> 0, faceObj.width, faceObj.height), 
                    debugContext.rotate(Math.PI / 2 - faceObj.angle), debugContext.translate(-faceObj.x, -faceObj.y)), 
                    !videoFaded && params.fadeVideo && (fadeVideo(), videoFaded = !0), this.status = "tracking", 
                    0 == faceObj.width || 0 == faceObj.height) params.retryDetection ? (headtrackerStatus("redetecting"), 
                    facetracker = new headtrackr.facetrackr.Tracker({
                        whitebalancing: !1,
                        debug: params.debug,
                        calcAngles: params.calcAngles
                    }), facetracker.init(canvasElement), faceFound = !1, headposition = void 0, videoFaded && (videoElement.style.opacity = 1, 
                    videoFaded = !1)) : (headtrackerStatus("lost"), this.stop()); else if (faceFound || (headtrackerStatus("found"), 
                    faceFound = !0), params.smoothing && (smoother.initialized || smoother.init(faceObj), 
                    faceObj = smoother.smooth(faceObj)), void 0 === headposition && params.headPosition) {
                        var stable = !1, headdiag = Math.sqrt(faceObj.width * faceObj.width + faceObj.height * faceObj.height);
                        headDiagonal.length < 6 ? headDiagonal.push(headdiag) : (headDiagonal.splice(0, 1), 
                        headDiagonal.push(headdiag), Math.max.apply(null, headDiagonal) - Math.min.apply(null, headDiagonal) < 5 && (stable = !0)), 
                        stable && (firstRun ? (headposition = void 0 === params.fov ? new headtrackr.headposition.Tracker(faceObj, canvasElement.width, canvasElement.height, {
                            distance_from_camera_to_screen: params.cameraOffset
                        }) : new headtrackr.headposition.Tracker(faceObj, canvasElement.width, canvasElement.height, {
                            fov: params.fov,
                            distance_from_camera_to_screen: params.cameraOffset
                        }), fov = headposition.getFOV(), firstRun = !1) : headposition = new headtrackr.headposition.Tracker(faceObj, canvasElement.width, canvasElement.height, {
                            fov: fov,
                            distance_from_camera_to_screen: params.cameraOffset
                        }), headposition.track(faceObj));
                    } else params.headPosition && headposition.track(faceObj);
                }
            }
            run && (detector = window.setTimeout(track, params.detectionInterval));
        }.bind(this), starter = function() {
            try {
                canvasContext.drawImage(videoElement, 0, 0, canvasElement.width, canvasElement.height);
                var canvasContent = headtrackr.getWhitebalance(canvasElement);
                canvasContent > 0 ? (run = !0, track()) : window.setTimeout(starter, 100);
            } catch (err) {
                window.setTimeout(starter, 100);
            }
        };
        this.start = function() {
            return this.initialized ? videoElement.currentTime > 0 && !videoElement.paused && !videoElement.ended ? (starter(), 
            !0) : (run = !0, videoElement.addEventListener("playing", starter, !1), !0) : !1;
        }, this.stop = function() {
            return window.clearTimeout(detector), run = !1, headtrackerStatus("stopped"), facetracker = void 0, 
            faceFound = !1, !0;
        }, this.stopStream = function() {
            void 0 !== this.stream && (void 0 !== this.stream.stop ? this.stream.stop() : void 0 !== this.stream.getVideoTracks && this.stream.getVideoTracks().forEach(function(t) {
                t.stop();
            }));
        ...
        var fadeVideo = function() {
            "" == videoElement.style.opacity ? (videoElement.style.opacity = .98, window.setTimeout(fadeVideo, 50)) : videoElement.style.opacity > .3 ? (videoElement.style.opacity -= .02, 
            window.setTimeout(fadeVideo, 50)) : videoElement.style.opacity = .3;
        };
    ...
    }), headtrackr.ccv = {}, headtrackr.ccv.grayscale = function(canvas) {
        ...
        var scale = Math.pow(2, 1 / (interval + 1)), next = interval + 1, scale_upto = Math.floor(Math.log(Math.min(cascade.width, cascade.height)) / Math.log(scale)), pyr = new Array(4 * (scale_upto + 2 * next));
        pyr[0] = canvas, pyr[0].data = pyr[0].getContext("2d").getImageData(0, 0, pyr[0].width, pyr[0].height).data;
        var i, j, k, x, y, q;
        ...
        pyr[4 * i].data = pyr[4 * i].getContext("2d").getImageData(0, 0, pyr[4 * i].width, pyr[4 * i].height).data;
        ...
        pyr[4 * i + 1].data = pyr[4 * i + 1].getContext("2d").getImageData(0, 0, pyr[4 * i + 1].width, pyr[4 * i + 1].height).data, 
        ...
        pyr[4 * i + 2].data = pyr[4 * i + 2].getContext("2d").getImageData(0, 0, pyr[4 * i + 2].width, pyr[4 * i + 2].height).data, 
        ...
        pyr[4 * i + 3].data = pyr[4 * i + 3].getContext("2d").getImageData(0, 0, pyr[4 * i + 3].width, pyr[4 * i + 3].height).data;
        for (j = 0; j < cascade.stage_classifier.length; j++) cascade.stage_classifier[j].orig_feature = cascade.stage_classifier[j].feature;
        var scale_x = 1, scale_y = 1, dx = [ 0, 1, 0, 1 ], dy = [ 0, 0, 1, 1 ], seq = [];
        ...
        for (var avggray, avgr, avgb, avgg, canvasContext = canvas.getContext("2d"), image = canvasContext.getImageData(0, 0, canvas.width, canvas.height), id = image.data, imagesize = image.width * image.height, r = g = b = 0, i = 0; imagesize > i; i++) r += id[4 * i], 
        g += id[4 * i + 1], b += id[4 * i + 2];
        return avgr = r / imagesize, avgg = g / imagesize, avgb = b / imagesize, avggray = (avgr + avgg + avgb) / 3;
    }, headtrackr.Smoother = function(alpha, interval) {
        function predict(time) {
            var retPos = [];
            if (this.interpolate) for (var step = time / interval, stepLo = step >> 0, ratio = alpha / (1 - alpha), a = (step - stepLo) * ratio, b = 2 + stepLo * ratio, c = 1 + stepLo * ratio, i = 0; sl > i; i++) retPos[i] = a * (sp[i] - sp2[i]) + b * sp[i] - c * sp2[i]; else for (var step = time / interval >> 0, ratio = alpha * step / (1 - alpha), a = 2 + ratio, b = 1 + ratio, i = 0; sl > i; i++) retPos[i] = a * sp[i] - b * sp2[i];
            return retPos;
        }
        var sp, sp2, sl, positions, updateTime = new Date();
        this.initialized = !1, this.interpolate = !1, this.init = function(initPos) {
            this.initialized = !0, sp = [ initPos.x, initPos.y, initPos.z, initPos.width, initPos.height ], 
            sp2 = sp, sl = sp.length;
        }, this.smooth = function(pos) {
            if (positions = [ pos.x, pos.y, pos.z, pos.width, pos.height ], this.initialized) {
                for (var i = 0; sl > i; i++) sp[i] = alpha * positions[i] + (1 - alpha) * sp[i], 
                sp2[i] = alpha * sp[i] + (1 - alpha) * sp2[i];
                updateTime = new Date();
                var msDiff = new Date() - updateTime, newPositions = predict(msDiff);
                return pos.x = newPositions[0], pos.y = newPositions[1], pos.z = newPositions[2], 
                pos.width = newPositions[3], pos.height = newPositions[4], pos;
            }
            return !1;
        };
    }, headtrackr.camshift = {}, headtrackr.camshift.Histogram = function(imgdata) {
        this.size = 4096;
        var i, x, r, g, b, il, bins = [];
        for (i = 0; i < this.size; i++) bins.push(0);
        for (x = 0, il = imgdata.length; il > x; x += 4) r = imgdata[x + 0] >> 4, g = imgdata[x + 1] >> 4, 
        b = imgdata[x + 2] >> 4, bins[256 * r + 16 * g + b] += 1;
        ...
        var i, j, val, vx, vy, a = [];
        for (i = x; w > i; i++) for (a = data[i], vx = i - x, j = y; h > j; j++) val = a[j], 
        vy = j - y, this.m00 += val, this.m01 += vy * val, this.m10 += vx * val, second && (this.m11 += vx * vy * val, 
        this.m02 += vy * vy * val, this.m20 += vx * vx * val);
        this.invM00 = 1 / this.m00, this.xc = this.m10 * this.invM00, this.yc = this.m01 * this.invM00, 
        this.mu00 = this.m00, this.mu01 = 0, this.mu10 = 0, second && (this.mu20 = this.m20 - this.m10 * this.xc, 
        this.mu02 = this.m02 - this.m01 * this.yc, this.mu11 = this.m11 - this.m01 * this.xc);
    }, headtrackr.camshift.Rectangle = function(x, y, w, h) {
        this.x = x, this.y = y, this.width = w, this.height = h, this.clone = function() {
            var c = new headtrackr.camshift.Rectangle();
            return c.height = this.height, c.width = this.width, c.x = this.x, c.y = this.y, 
            c;
        };
    }, headtrackr.camshift.Tracker = function(params) {
        function camShift(frame) {
            var w = frame.width, h = frame.height, m = meanShift(frame), a = m.mu20 * m.invM00, c = m.mu02 * m.invM00;
            if (params.calcAngles) {
                var b = m.mu11 * m.invM00, d = a + c, e = Math.sqrt(4 * b * b + (a - c) * (a - c));
                _trackObj.width = Math.sqrt(.5 * (d - e)) << 2, _trackObj.height = Math.sqrt(.5 * (d + e)) << 2, 
                _trackObj.angle = Math.atan2(2 * b, a - c + e), _trackObj.angle < 0 && (_trackObj.angle = _trackObj.angle + Math.PI);
            } else _trackObj.width = Math.sqrt(a) << 2, _trackObj.height = Math.sqrt(c) << 2, 
            ...
            _searchWindow.width = Math.floor(1.1 * _trackObj.width), _searchWindow.height = Math.floor(1.1 * _trackObj.height);
        }
        function meanShift(frame) {
            var w = frame.width, h = frame.height, imgData = frame.data, curHist = new headtrackr.camshift.Histogram(imgData), weights = getWeights(_modelHist, curHist);
            _pdf = getBackProjectionData(imgData, frame.width, frame.height, weights);
            var m, x, y, i, wadx, wady, wadw, wadh, meanShiftIterations = 10, prevx = _searchWindow.x, prevy = _searchWindow.y;
            for (i = 0; meanShiftIterations > i; i++) {
                if (wadx = Math.max(_searchWindow.x, 0), wady = Math.max(_searchWindow.y, 0), wadw = Math.min(wadx + _searchWindow.width, w), 
                wadh = Math.min(wady + _searchWindow.height, h), m = new headtrackr.camshift.Moments(_pdf, wadx, wady, wadw, wadh, i == meanShiftIterations - 1), 
                x = m.xc, y = m.yc, _searchWindow.x += x - _searchWindow.width / 2 >> 0, _searchWindow.y += y - _searchWindow.height / 2 >> 0, 
                _searchWindow.x == prevx && _searchWindow.y == prevy) {
                    m = new headtrackr.camshift.Moments(_pdf, wadx, wady, wadw, wadh, !0);
                    break;
                }
                prevx = _searchWindow.x, prevy = _searchWindow.y;
            }
            return _searchWindow.x = Math.max(0, Math.min(_searchWindow.x, w)), _searchWindow.y = Math.max(0, Math.min(_searchWindow.y, h)), 
            m;
        }
        function getWeights(mh, ch) {
            for (var p, weights = [], i = 0; 4096 > i; i++) p = 0 != ch.getBin(i) ? Math.min(mh.getBin(i) / ch.getBin(i), 1) : 0, 
            weights.push(p);
            return weights;
        }
        function getBackProjectionData(imgData, idw, idh, weights, hsMap) {
            var x, y, r, g, b, pos, data = [], a = [];
            for (x = 0; idw > x; x++) {
                for (a = [], y = 0; idh > y; y++) pos = 4 * (y * idw + x), r = imgData[pos] >> 4, 
                g = imgData[pos + 1] >> 4, b = imgData[pos + 2] >> 4, a.push(weights[256 * r + 16 * g + b]);
                data[x] = a;
            }
            return data;
        }
        void 0 === params && (params = {}), void 0 === params.calcAngles && (params.calcAngles = !0);
        var _modelHist, _pdf, _searchWindow, _trackObj, _canvasCtx, _canvasw, _canvash;
        this.getSearchWindow = function() {
            ...
            var x, y, val, weights = _pdf, w = _canvasw, h = _canvash, img = _canvasCtx.createImageData(w, h), imgData = img.data;
            for (x = 0; w > x; x++) for (y = 0; h > y; y++) val = Math.floor(255 * weights[x][y]), 
            pos = 4 * (y * w + x), imgData[pos] = val, imgData[pos + 1] = val, imgData[pos + 2] = val, 
            imgData[pos + 3] = 255;
            return img;
        }, this.initTracker = function(canvas, trackedArea) {
            _canvasCtx = canvas.getContext("2d");
            var taw = trackedArea.width, tah = trackedArea.height, tax = trackedArea.x, tay = trackedArea.y, trackedImg = _canvasCtx.getImageData(tax, tay, taw, tah);
            _modelHist = new headtrackr.camshift.Histogram(trackedImg.data), _searchWindow = trackedArea.clone(), 
            _trackObj = new headtrackr.camshift.TrackObj();
        }, this.track = function(canvas) {
            var canvasCtx = canvas.getContext("2d");
            _canvash = canvas.height, _canvasw = canvas.width;
            var imgData = canvasCtx.getImageData(0, 0, canvas.width, canvas.height);
            0 != imgData.width && 0 != imgData.height && camShift(imgData);
        };
        ...
        this.height = 0, this.width = 0, this.angle = 0, this.x = 0, this.y = 0, this.clone = function() {
            ...
            return c.height = this.height, c.width = this.width, c.angle = this.angle, c.x = this.x, 
            c.y = this.y, c;
        };
    }, headtrackr.facetrackr = {}, headtrackr.facetrackr.Tracker = function(params) {
        function doVJDetection() {
            var start = new Date().getTime(), ccvCanvas = document.createElement("canvas");
            ccvCanvas.width = _inputcanvas.width, ccvCanvas.height = _inputcanvas.height, ccvCanvas.getContext("2d").drawImage(_inputcanvas, 0, 0, ccvCanvas.width, ccvCanvas.height);
            var candidate, comp = headtrackr.ccv.detect_objects(headtrackr.ccv.grayscale(ccvCanvas), headtrackr.cascade, 5, 1), diff = new Date().getTime() - start;
            comp.length > 0 && (candidate = comp[0]);
            for (var i = 1; i < comp.length; i++) comp[i].confidence > candidate.confidence && (candidate = comp[i]);
            var result = new headtrackr.facetrackr.TrackObj();
            return void 0 !== candidate && (result.width = candidate.width, result.height = candidate.height, 
            result.x = candidate.x, result.y = candidate.y, result.confidence = candidate.confidence), 
            result.time = diff, result.detection = "VJ", result;
        }
        function doCSDetection() {
            var start = new Date().getTime();
            _cstracker.track(_inputcanvas);
            var csresult = _cstracker.getTrackObj();
            params.debug && params.debug.getContext("2d").putImageData(_cstracker.getBackProjectionImg(), 0, 0);
            var diff = new Date().getTime() - start, result = new headtrackr.facetrackr.TrackObj();
            return result.width = csresult.width, result.height = csresult.height, result.x = csresult.x, 
            result.y = csresult.y, result.angle = csresult.angle, result.confidence = 1, result.time = diff, 
            result.detection = "CS", result;
        }
        function checkWhitebalance() {
            var result = new headtrackr.facetrackr.TrackObj();
            return result.wb = headtrackr.getWhitebalance(_inputcanvas), result.detection = "WB", 
            result;
        }
        if (params || (params = {}), void 0 === params.sendEvents && (params.sendEvents = !0), 
        void 0 === params.whitebalancing && (params.whitebalancing = !0), void 0 === params.debug ? params.debug = !1 : "CANVAS" != params.debug.tagName && (params.debug = !1), 
        params.whitebalancing) var _currentDetection = "WB"; else var _currentDetection = "VJ";
        void 0 == params.calcAngles && (params.calcAngles = !1);
        var _inputcanvas, _curtracked, _cstracker, _confidenceThreshold = -10, previousWhitebalances = [], pwbLength = 15;
        this.init = function(inputcanvas) {
            _inputcanvas = inputcanvas, _cstracker = new headtrackr.camshift.Tracker({
                calcAngles: params.calcAngles
            });
        }, this.track = function() {
            var result;
            if ("WB" == _currentDetection ? result = checkWhitebalance() : "VJ" == _currentDetection ? result = doVJDetection() : "CS" == _currentDetection && (result = doCSDetection()), 
            "WB" == result.detection && (previousWhitebalances.length >= pwbLength && previousWhitebalances.pop(), 
            previousWhitebalances.unshift(result.wb), previousWhitebalances.length == pwbLength)) {
                var max = Math.max.apply(null, previousWhitebalances), min = Math.min.apply(null, previousWhitebalances);
                2 > max - min && (_currentDetection = "VJ");
            }
            if ("VJ" == result.detection && result.confidence > _confidenceThreshold) {
            ...
            if (_curtracked = result, "CS" == result.detection && params.sendEvents) {
                var evt = document.createEvent("Event");
                evt.initEvent("facetrackingEvent", !0, !0), evt.height = result.height, evt.width = result.width, 
                evt.angle = result.angle, evt.x = result.x, evt.y = result.y, evt.confidence = result.confidence, 
                evt.detection = result.detection, evt.time = result.time, document.dispatchEvent(evt);
            }
        }, this.getTrackingObject = function() {
            return _curtracked.clone();
        };
    }, headtrackr.facetrackr.TrackObj = function() {
        this.height = 0, this.width = 0, this.angle = 0, this.x = 0, this.y = 0, this.confidence = -1e4, 
        this.detection = "", this.time = 0, this.clone = function() {
            var c = new headtrackr.facetrackr.TrackObj();
            return c.height = this.height, c.width = this.width, c.angle = this.angle, c.x = this.x, 
            c.y = this.y, c.confidence = this.confidence, c.detection = this.detection, c.time = this.time, 
            c;
        };
    }, headtrackr.Ui = function() {
        var timeout, d = document.createElement("div"), d2 = document.createElement("div"), p = document.createElement("p");
        d.setAttribute("id", "headtrackerMessageDiv"), d.style.left = "20%", d.style.right = "20%", 
        d.style.top = "30%", d.style.fontSize = "90px", d.style.color = "#777", d.style.position = "absolute", 
        d.style.fontFamily = "Helvetica, Arial, sans-serif", d.style.zIndex = "100002", 
        d2.style.marginLeft = "auto", d2.style.marginRight = "auto", d2.style.width = "100%", 
        d2.style.textAlign = "center", d2.style.color = "#fff", d2.style.backgroundColor = "#444", 
        d2.style.opacity = "0.5", p.setAttribute("id", "headtrackerMessage"), d2.appendChild(p), 
        d.appendChild(d2), document.body.appendChild(d);
        var supportMessages = {
        ...
        document.addEventListener("headtrackrStatus", function(event) {
            if (event.status in statusMessages) {
                if (window.clearTimeout(timeout), !override) {
                    var messagep = document.getElementById("headtrackerMessage");
                    messagep.innerHTML = statusMessages[event.status], timeout = window.setTimeout(function() {
                        messagep.innerHTML = "";
                    }, 3e3);
                }
            } else if (event.status in supportMessages) {
                override = !0, window.clearTimeout(timeout);
                var messagep = document.getElementById("headtrackerMessage");
                messagep.innerHTML = supportMessages[event.status], window.setTimeout(function() {
                    messagep.innerHTML = "added fallback video for demo";
                }, 2e3), window.setTimeout(function() {
                    messagep.innerHTML = "", override = !1;
                }, 4e3);
            }
        }, !0);
    }, headtrackr.headposition = {}, headtrackr.headposition.Tracker = function(facetrackrObj, camwidth, camheight, params) {
        if (params || (params = {}), void 0 === params.edgecorrection) var edgecorrection = !0; else var edgecorrection = params.edgecorrection;
        this.camheight_cam = camheight, this.camwidth_cam = camwidth;
        var head_width_cm = 16, head_height_cm = 19, head_small_angle = Math.atan(head_width_cm / head_height_cm), head_diag_cm = Math.sqrt(head_width_cm * head_width_cm + head_height_cm * head_height_cm), sin_hsa = Math.sin(head_small_angle), cos_hsa = Math.cos(head_small_angle), tan_hsa = Math.tan(head_small_angle), init_width_cam = facetrackrObj.width, init_height_cam = facetrackrObj.height, head_diag_cam = Math.sqrt(init_width_cam * init_width_cam + init_height_cam * init_height_cam);
        if (void 0 === params.fov) {
            var head_width_cam = sin_hsa * head_diag_cam, camwidth_at_default_face_cm = this.camwidth_cam / head_width_cam * head_width_cm;
            if (void 0 === params.distance_to_screen) var distance_to_screen = 60; else var distance_to_screen = params.distance_to_screen;
            var fov_width = 2 * Math.atan(camwidth_at_default_face_cm / 2 / distance_to_screen);
        } else var fov_width = params.fov * Math.PI / 180;
        var x, y, z, tan_fov_width = 2 * Math.tan(fov_width / 2);
        this.track = function(facetrackrObj) {
            var w = facetrackrObj.width, h = facetrackrObj.height, fx = facetrackrObj.x, fy = facetrackrObj.y;
            if (edgecorrection) {
                var margin = 11, leftDistance = fx - w / 2, rightDistance = this.camwidth_cam - (fx + w / 2), topDistance = fy - h / 2, bottomDistance = this.camheight_cam - (fy + h / 2), onVerticalEdge = margin > leftDistance || margin > rightDistance, onHorizontalEdge = margin > topDistance || margin > bottomDistance;
                if (onHorizontalEdge) if (onVerticalEdge) {
                    var onLeftEdge = margin > leftDistance, onTopEdge = margin > topDistance;
                    fx = onLeftEdge ? w - head_diag_cam * sin_hsa / 2 : fx - w / 2 + head_diag_cam * sin_hsa / 2, 
                    fy = onTopEdge ? h - head_diag_cam * cos_hsa / 2 : fy - h / 2 + head_diag_cam * cos_hsa / 2;
                } else if (margin > topDistance) {
                    var originalWeight = topDistance / margin, estimateWeight = (margin - topDistance) / margin;
                    fy = h - (originalWeight * (h / 2) + estimateWeight * (w / tan_hsa / 2)), head_diag_cam = estimateWeight * (w / sin_hsa) + originalWeight * Math.sqrt(w * w + h * h);
                } else {
                    var originalWeight = bottomDistance / margin, estimateWeight = (margin - bottomDistance) / margin;
                ...
                } else if (onVerticalEdge) if (margin > leftDistance) {
                    var originalWeight = leftDistance / margin, estimateWeight = (margin - leftDistance) / margin;
                    ...
                    var originalWeight = rightDistance / margin, estimateWeight = (margin - rightDistance) / margin;
                    head_diag_cam = estimateWeight * (h / cos_hsa) + originalWeight * Math.sqrt(w * w + h * h), 
                    fx = fx - w / 2 + (originalWeight * (w / 2) + estimateWeight * (h * tan_hsa / 2));
                } else head_diag_cam = Math.sqrt(w * w + h * h);
            } else head_diag_cam = Math.sqrt(w * w + h * h);
            z = head_diag_cm * this.camwidth_cam / (tan_fov_width * head_diag_cam), x = -(fx / this.camwidth_cam - .5) * z * tan_fov_width, 
            y = -(fy / this.camheight_cam - .5) * z * tan_fov_width * (this.camheight_cam / this.camwidth_cam), 
            y += void 0 === params.distance_from_camera_to_screen ? 11.5 : params.distance_from_camera_to_screen;
            var evt = document.createEvent("Event");
            return evt.initEvent("headtrackingEvent", !0, !0), evt.x = x, evt.y = y, evt.z = z, 
            document.dispatchEvent(evt), new headtrackr.headposition.TrackObj(x, y, z);
        }, this.getTrackerObj = function() {
            return new headtrackr.headposition.TrackObj(x, y, z);
        }, this.getFOV = function() {
            return 180 * fov_width / Math.PI;
        };
    }, headtrackr.headposition.TrackObj = function(x, y, z) {
        this.x = x, this.y = y, this.z = z, this.clone = function() {
            var c = new headtrackr.headposition.TrackObj();
            return c.x = this.x, c.y = this.y, c.z = this.z, c;
        };
    }, headtrackr.controllers = {}, headtrackr.controllers.three = {}, headtrackr.controllers.three.realisticAbsoluteCameraControl = function(camera, scaling, fixedPosition, lookAt, params) {
        if (void 0 === params && (params = {}), void 0 === params.screenHeight) var screenHeight_cms = 20; else var screenHeight_cms = params.screenHeight;
        void 0 === params.damping && (params.damping = 1), camera.position.x = fixedPosition[0], 
        camera.position.y = fixedPosition[1], camera.position.z = fixedPosition[2], camera.lookAt(lookAt);
        var wh = screenHeight_cms * scaling, ww = wh * camera.aspect;
        document.addEventListener("headtrackingEvent", function(event) {
            var xOffset = event.x > 0 ? 0 : 2 * -event.x * params.damping * scaling, yOffset = event.y < 0 ? 0 : 2 * event.y * params.damping * scaling;
            camera.setViewOffset(ww + Math.abs(2 * event.x * params.damping * scaling), wh + Math.abs(event.y * params.damping * 2 * scaling), xOffset, yOffset, ww, wh), 
            camera.position.x = fixedPosition[0] + event.x * scaling * params.damping, camera.position.y = fixedPosition[1] + event.y * scaling * params.damping, 
            camera.position.z = fixedPosition[2] + event.z * scaling, camera.fov = 360 * Math.atan((wh / 2 + Math.abs(event.y * scaling * params.damping)) / Math.abs(event.z * scaling)) / Math.PI, 
            camera.updateProjectionMatrix();
        }, !1);
    }, headtrackr.controllers.three.realisticRelativeCameraControl = function(camera, scaling, relativeFixedDistance, params) {
        if (void 0 === params && (params = {}), void 0 === params.screenHeight) var screenHeight_cms = 20; else var screenHeight_cms = params.screenHeight;
        var scene = camera.parent, offset = new THREE.Object3D();
        offset.position.set(0, 0, 0), offset.add(camera), scene.add(offset);
        var wh = screenHeight_cms * scaling, ww = wh * camera.aspect;
        document.addEventListener("headtrackingEvent", function(event) {
            var xOffset = event.x > 0 ? 0 : 2 * -event.x * scaling, yOffset = event.y > 0 ? 0 : 2 * -event.y * scaling;
            camera.setViewOffset(ww + Math.abs(2 * event.x * scaling), wh + Math.abs(2 * event.y * scaling), xOffset, yOffset, ww, wh), 
            offset.rotation = camera.rotation, offset.position.x = 0, offset.position.y = 0, 
            offset.position.z = 0, offset.translateX(event.x * scaling), offset.translateY(event.y * scaling), 
            offset.translateZ(event.z * scaling + relativeFixedDistance), camera.fov = 360 * Math.atan((wh / 2 + Math.abs(event.y * scaling)) / Math.abs(event.z * scaling)) / Math.PI, 
            camera.updateProjectionMatrix();
        }, !1);
    }, headtrackr;
});
